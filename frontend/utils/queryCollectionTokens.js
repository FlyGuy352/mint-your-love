...
export default async function (ownedTokenIds, linkedCollectionIds) {
  ...
  const { data: { nftMinteds } } = await client.query({
    query: gql`{
  ...
  const { data: { collections: ownedCollections } } = await client.query({
    query: gql`{
  ...
  const { data: { collections: linkedCollections } } = await client.query({
    query: gql`{
